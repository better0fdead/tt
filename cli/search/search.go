...
func getBundles(rawBundleInfoList map[string]interface{}, flags SearchFlags) (BundleInfoSlice,
	error) {
	...
	for release, pkgs := range rawBundleInfoList {
		if pkgArr, ok := pkgs.([]interface{}); ok {
			for _, pkg := range pkgArr {
				pkgStr := fmt.Sprintf("%v", pkg)
				parsedData := util.FindNamedMatches(re, pkgStr)
				if len(parsedData) == 0 {
					continue
				}
				...
				switch flags {
				case SearchRelease:
					if strings.Contains(pkgStr, "-debug-") {
						continue
					}
				case SearchDebug:
					if !strings.Contains(pkgStr, "-debug-") {
