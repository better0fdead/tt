...
func tntIoGetPkgVersions(cliOpts *config.CliOpts, searchCtx SearchCtx) (map[string]interface{},
	string, error) {
	var token string
	var query string
	var osType string
	...
	var apiReply map[string]interface{}
	...
	for release, pkgs := range apiReply {
		if pkgArr, ok := pkgs.([]interface{}); ok {
			...
			for _, pkg := range pkgArr {
				...
				if !strings.HasSuffix(pkgStr, ".sha256") {
